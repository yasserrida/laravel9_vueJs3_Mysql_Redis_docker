...
import { getAll, store, update, deleteItem } from '/@src/api/gammesApi'
...
const getGammes = async (): Promise<void> => {
  gammes.value = await getAll()
}

const editItem = (event: any): void => {
  const element = gammes.value.find((item: any): boolean => item.id == event.data.id)
  if (element) {
  ...
  <div>
    ...
    <div class="my-4">
      ...
      <VModal
        ...
        actions="right"
        size="large"
        :title="state.id ? 'Modifier Gamme' : 'Nouvelle Gamme'"
